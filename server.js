...
app.post('/api/frame', (req, res) => {
  try {
    ...
    if (buttonIndex === 1 && gameState.currentRound === 1 && gameState.history.length === 0) {
      // User clicked "Play Game" on the welcome screen
      responseFrame = {
        version: "vNext",
        imageUrl: `${req.protocol}://${req.get('host')}/public/images/choose.png`,
        buttons: [
          {
            label: "✊ Rock",
            action: "post"
    ...
    } else if (buttonIndex >= 1 && buttonIndex <= 3 && gameState.currentRound <= gameState.maxRounds) {
      ...
      if (isGameOver) {
        ...
        responseFrame = {
          version: "vNext",
          imageUrl: `${req.protocol}://${req.get('host')}/public/images/result-${playerWon ? 'win' : 'lose'}.png`,
          buttons: [
            {
              label: "Play Again",
              action: "post"
        ...
        responseFrame = {
          version: "vNext",
          imageUrl: `${req.protocol}://${req.get('host')}/public/images/round-${playerChoice}-${computerChoice}.png`,
          buttons: [
            {
              label: "Next Round",
              action: "post"
    ...
    } else if (buttonIndex === 1 && (gameState.currentRound > gameState.maxRounds || 
              gameState.playerScore >= 3 || 
              gameState.computerScore >= 3)) {
    ...
    } else if (buttonIndex === 2 && (gameState.currentRound > gameState.maxRounds || 
              gameState.playerScore >= 3 || 
              gameState.computerScore >= 3)) {
      ...
      responseFrame = {
        version: "vNext",
        imageUrl: `${req.protocol}://${req.get('host')}/public/images/cast-win.png`,
        buttons: [
          {
            label: "Play Again",
            action: "post"
      ...
      responseFrame = {
        version: "vNext",
        imageUrl: `${req.protocol}://${req.get('host')}/public/images/welcome.png`,
        buttons: [
          {
            label: "Play Game",
            action: "post"
...
setInterval(() => {
  ...
  gameStates.forEach((state, sessionId) => {
    if (now - state.lastUpdated > 3600000) { // 1 hour
      gameStates.delete(sessionId);
    }
  });
}, 3600000); // 1 hour
